<? //универсальный код отображения страницы с добавлением, редактированием, удалением записей (1.1)

modUserAccess_check(0);

include_once("mod_strings.php");
//---провека наличия требуемых таблиц и авто-создание при отсутствии
        $sqlCheck = "SHOW TABLES LIKE 'setup_plc'";
        $resCheck = mysql_query($sqlCheck);
        $countCheck = 0;
        while ($rCheck = mysql_fetch_array($resCheck)) $countCheck++;
        if($countCheck==0) {
          $sqlCheck = "
  	  CREATE TABLE `setup_plc` (
	    `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id',
	    `title` enum('icp7017','adam4017') NOT NULL DEFAULT 'icp7017' COMMENT 'Устройство',
	    `addr` varchar(127) NOT NULL COMMENT 'Адрес',
	    `active` enum('0','1') NOT NULL DEFAULT '1' COMMENT 'Использовать',
	    PRIMARY KEY (`id`)
...
$title_view_table="Настройка ПЛК";
$icon_view_table="zwicon-table";
$sql_view_table="
SELECT
	setup_plc.id AS id,
	setup_plc.title AS title,
	setup_plc.addr AS addr,
	setup_plc.active AS active
FROM 
	setup_plc
WHERE
1
ORDER BY setup_plc.id DESC;
";

$title_view_detail="Редактирование";
$icon_view_detail="zwicon-document";

$sql_main_table="setup_plc"; //для создания/удаления записей
$sql_view_detail="
SELECT
	setup_plc.id AS id,
	setup_plc.title AS title,
...
FROM 
	setup_plc 
WHERE
	setup_plc.id=$_GET[id]
";

$title_add_item="Добавление нового параметра";
$icon_add_item="zwicon-document";
...
{ //функция вывода таблицы согласно запросу
    ...
    <section class="content">
        <div class="card">
            <div class="card-body">
                ...
                <div style="margin-top: -20px;">
                    <a href="<?= $pg; ?>?additem=1" class="btn btn-theme btn--icon"><i class="zwicon-plus"></i></a>
                </div>
                <h6></h6>
		...
		if (count($tables)==1) $tables=$tables[0];
                                    ...
                                    if ($key == "id") { ?>
                                        <td>
                                            <a href="/<?= $pg; ?>?id=<?= $rT[id]; ?>" class="btn btn-theme btn--icon"><i
                                                        class="zwicon-document"></i></a>
                                            <a href="<?=$pg?>?delete_item_id=<?=$rT[id];?>" onClick="return confirm('Подтвердите удаление?')" class="btn btn-theme btn--icon"><i class="zwicon-close"></i></a>
                                        </td>
                                        <?
                                    }//==id
                                }//is_numeric
