...
class PhpSerial
{
     ...
     * @return bool
     */
    public function deviceSet($device,$baud)
    {
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
            if ($this->_os === "linux") {
                ...
                if (preg_match("@^COM(\\d+):?$@i", $device, $matches)
                        and $this->_exec(
                            exec("mode " . $device . " BAUD=".$baud." PARITY=N DATA=8 STOP=1")
                        ) === 0
                ) {
                    $this->_winDevice = "COM" . $matches[1];
                    $this->_device = "COM" . $matches[1]; //для nix систем //to=on xon=off odsr=off octs=off dtr=off rts=off idsr=off
                    //$this->_device = "\\.com" . $matches[1]; //для nix систем
                    $this->_dState = SERIAL_DEVICE_SET;
                    return true;
                }
            }
     ...
     * @return bool
     */
    public function deviceOpen($mode = "w+") // "r+b" заменил на "w+"
    {
        if ($this->_dState === SERIAL_DEVICE_OPENED) {
    ...
    public function readPort($count = 0)
    {
        ...
        if ($this->_os === "linux" || $this->_os === "osx") {
            ...
            if ($count !== 0) {  //($count !== 0)
                ...
                do {
                    $content .= fread($this->_dHandle, 128); //тут зависает иногда?
                } while (($i += 128) === strlen($content));
            }
